...
public:
  ...
  /** default constructor */
  TempQueue() {items = new queue<std::pair<Head, Tail> >();};
  TempQueue(const TempQueue& q) {items = new queue<std::pair<Head,Tail> >(*(q.items));};
  ~TempQueue() {/*std::cout << "TempQueue::dtor!" << std::endl;*/delete items;};
