    ...
    size_t TransactionManager::Transaction::load(
        ...
        for (size_t i = 1; i < column_names.size(); ++i) {
            if (nums_values[i] != previousNumValues) {
                std::stringstream sserr;
                sserr << "Transaction::load(@" << __FILE__ << ':' << __LINE__ << ") For table \"" << tableName << "\": Column sizes don't match! previous (" << (i - 1) << ", " << column_names[i - 1]
                        << ") = " << previousNumValues << ", current (" << i << ", " << column_names[i] << ") = " << nums_values[i];
