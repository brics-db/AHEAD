...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < CONFIG.NUM_RUNS; ++i) {
        ...
        MEASURE_OP_TUPLE(tuple6,
                v2::bat::ops::matchjoinAN(bat5, batLOenc, std::get<13>(*v2_resoid_t::As), std::get<13>(*v2_resoid_t::Ainvs), std::get<14>(*v2_resint_t::As), std::get<14>(*v2_resint_t::Ainvs))); // only those lo_orderdates where lo_quantity... and lo_discount
        delete bat5;
        ...
        MEASURE_OP_TUPLE(tuple9,
                (v2::bat::ops::matchjoinAN(bat8, batDDenc, std::get<12>(*v2_resoid_t::As), std::get<12>(*v2_resoid_t::Ainvs), std::get<13>(*v2_resint_t::As), std::get<13>(*v2_resint_t::Ainvs)))); // only those d_datekey where d_year...
        delete bat8;
        ...
        MEASURE_OP_TUPLE(tupleB,
                (v2::bat::ops::hashjoinAN(std::get<0>(tuple6), batA, std::get<11>(*v2_resoid_t::As), std::get<11>(*v2_resoid_t::Ainvs), std::get<10>(*v2_resoid_t::As),
                        std::get<10>(*v2_resoid_t::Ainvs)))); // only those lineorders where lo_quantity... and lo_discount... and d_year...
        ...
        MEASURE_OP_TUPLE(tupleD,
                (v2::bat::ops::matchjoinAN(batC, batLEenc, std::get<9>(*v2_resoid_t::As), std::get<9>(*v2_resoid_t::Ainvs), std::get<12>(*v2_resint_t::As), std::get<12>(*v2_resint_t::Ainvs))));CLEAR_HASHJOIN_AN(
                tupleD);
        MEASURE_OP_TUPLE(tupleE,
                (v2::bat::ops::matchjoinAN(batC, std::get<0>(tuple4), std::get<8>(*v2_resoid_t::As), std::get<8>(*v2_resoid_t::Ainvs), std::get<3>(*v2_restiny_t::As),
