...
main (int argc, char** argv) {
    ...
    for (size_t i = 0; i < CONFIG.NUM_RUNS; ++i) {
        ...
        for (size_t k = 0; k < MODULARITY; ++k) {
            ...
            MEASURE_OP(bat1, v2::bat::ops::select<std::less>(batLQs[k], 25)); // lo_quantity < 25
            MEASURE_OP(bat2, v2::bat::ops::select(batLDs[k], 1, 3)); // lo_discount between 1 and 3
            ...
            MEASURE_OP(bat7, v2::bat::ops::select<std::equal_to>(batDYs[k], 1993)); // d_year = 1993
            auto bat8 = bat7->mirror_head(); // prepare joined selection over d_year and d_datekey
            delete bat7;
            MEASURE_OP(bat9, v2::bat::ops::matchjoin(bat8, batDDs[k])); // only those d_datekey where d_year...
            delete bat8;
            ...
            delete batB;
            MEASURE_OP(batD, v2::bat::ops::matchjoin(batC, batLEs[k]));
