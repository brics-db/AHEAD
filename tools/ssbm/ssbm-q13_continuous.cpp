...
int main(
    ...
    return ssb::run(argc, argv, [] (int argc, char ** argv) -> int {
        ...
        for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
            ...
            MEASURE_OP_TUPLE(pair2, (selectAN<std::greater_equal, std::less_equal, ahead::and_is>(batLDenc, 5ull * batLDenc->tail.metaData.AN_A, 7ull * batLDenc->tail.metaData.AN_A)));// lo_discount between 5 and 7
            ...
            delete pair2.first;
            MEASURE_OP_TUPLE(tuple4, matchjoinAN(bat3, batZ));// join selection
            delete bat3;
            delete batZ;
            ...
            delete pair7.first;
            MEASURE_OP_PAIR(pair9, selectAN<std::equal_to>(batDWenc, 6u * batDWenc->tail.metaData.AN_A));// d_weeknuminyear = 6
            CLEAR_SELECT_AN(pair9);
            auto batY = pair9.first->mirror_head();
            delete pair9.first;
            MEASURE_OP_TUPLE(tupleA, matchjoinAN(bat8, batY));
            delete bat8;
            delete batY;
            ...
            MEASURE_OP_TUPLE(tupleH, matchjoinAN(bat5, batLDenc));
            CLEAR_JOIN_AN(tupleH);
            delete bat5;
            MEASURE_OP_TUPLE(tupleI, matchjoinAN(batF, std::get<0>(tupleH)));
