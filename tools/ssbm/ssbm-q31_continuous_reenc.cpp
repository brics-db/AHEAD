...
int main(
    ...
    for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
        ...
        // grouping
        MEASURE_OP_TUPLE(tupleGD, groupbyAN(std::get<0>(tupleAD))); // automatic encoding of the resoid_t tails
        CLEAR_GROUPBY_UNARY_AN(tupleGD);
        MEASURE_OP_TUPLE(tupleGS, groupbyAN(std::get<0>(tupleAS), std::get<0>(tupleGD), std::get<1>(tupleGD)->size())); // automatic encoding of the resoid_t tails
        CLEAR_GROUPBY_BINARY_AN(tupleGS);
        delete std::get<0>(tupleGD);
        delete std::get<1>(tupleGD);
        MEASURE_OP_TUPLE(tupleGC, groupbyAN(std::get<0>(tupleAC), std::get<0>(tupleGS), std::get<1>(tupleGS)->size())); // automatic encoding of the resoid_t tails
