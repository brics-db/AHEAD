...
main (int argc, char** argv) {
    ...
    for (size_t i = 0; i < CONFIG.NUM_RUNS; ++i) {
        ...
        MEASURE_OP_PAIR(pair1, v2::bat::ops::selectAN(batLQenc, 26 * batLQenc->tail.metaData.AN_A, 35 * batLQenc->tail.metaData.AN_A, std::get<6>(*v2_restiny_t::As), std::get<6>(*v2_restiny_t::Ainvs))); // lo_quantity between 26 and 35
        delete pair1.second;
        MEASURE_OP_PAIR(pair2, v2::bat::ops::selectAN(batLDenc, 5 * batLDenc->tail.metaData.AN_A, 7 * batLDenc->tail.metaData.AN_A, std::get<5>(*v2_restiny_t::As), std::get<5>(*v2_restiny_t::Ainvs))); // lo_discount between 5 and 7
        delete pair2.second;
        auto bat3 = pair1.first->mirror_head(); // prepare joined selection (select from lineorder where lo_quantity... and lo_discount)
        delete pair1.first;
        MEASURE_OP_TUPLE(tuple4, v2::bat::ops::matchjoinAN(bat3, pair2.first, std::get<14>(*v2_resoid_t::As), std::get<14>(*v2_resoid_t::Ainvs), std::get<4>(*v2_restiny_t::As), std::get<4>(*v2_restiny_t::Ainvs))); // join selection
        delete pair2.first;
        delete bat3;
        ...
        MEASURE_OP_TUPLE(tuple6, v2::bat::ops::matchjoinAN(bat5, batLOenc, std::get<13>(*v2_resoid_t::As), std::get<13>(*v2_resoid_t::Ainvs), std::get<12>(*v2_resoid_t::As), std::get<12>(*v2_resoid_t::Ainvs))); // only those lo_orderdates where lo_quantity... and lo_discount
        delete bat5;
        CLEAR_HASHJOIN_AN(tuple6);

        // 2) select from date (join inbetween to reduce the number of lines we touch in total)
        MEASURE_OP_PAIR(pair7, v2::bat::ops::selectAN<std::equal_to>(batDYenc, 1994 * batDYenc->tail.metaData.AN_A)); // d_year = 1994
        delete pair7.second;
        auto bat8 = pair7.first->mirror_head(); // prepare joined selection over d_year and d_weeknuminyear
        delete pair7.first;
        MEASURE_OP_PAIR(pair9, v2::bat::ops::selectAN<std::equal_to>(batDWenc, 6 * batDWenc->tail.metaData.AN_A)); // d_weeknuminyear = 6
        delete pair9.second;
        MEASURE_OP_TUPLE(tupleA, v2::bat::ops::matchjoinAN(bat8, pair9.first, std::get<11>(*v2_resoid_t::As), std::get<11>(*v2_resoid_t::Ainvs), std::get<14>(*v2_resshort_t::As), std::get<14>(*v2_resshort_t::Ainvs)));
        delete bat8;
        delete pair9.first;
        CLEAR_HASHJOIN_AN(tupleA);
        auto batB = std::get<0>(tupleA)->mirror_head();
        delete std::get<0>(tupleA);
        MEASURE_OP_TUPLE(tupleC, v2::bat::ops::matchjoinAN(batB, batDDenc, std::get<10>(*v2_resoid_t::As), std::get<10>(*v2_resoid_t::Ainvs), std::get<14>(*v2_resint_t::As), std::get<14>(*v2_resint_t::Ainvs))); // only those d_datekey where d_year and d_weeknuminyear...
        delete batB;
        ...
        MEASURE_OP_TUPLE(tupleE, v2::bat::ops::hashjoinAN(std::get<0>(tuple6), batD, std::get<9>(*v2_resoid_t::As), std::get<9>(*v2_resoid_t::Ainvs), std::get<13>(*v2_resint_t::As), std::get<13>(*v2_resint_t::Ainvs))); // only those lineorders where lo_quantity... and lo_discount... and d_year...
        ...
        MEASURE_OP_TUPLE(tupleG, v2::bat::ops::matchjoinAN(batF, batLEenc, std::get<8>(*v2_resoid_t::As), std::get<8>(*v2_resoid_t::Ainvs), std::get<12>(*v2_resint_t::As), std::get<12>(*v2_resint_t::Ainvs)));
        CLEAR_HASHJOIN_AN(tupleG);
        MEASURE_OP_TUPLE(tupleH, v2::bat::ops::matchjoinAN(batF, std::get<0>(tuple4), std::get<7>(*v2_resoid_t::As), std::get<7>(*v2_resoid_t::Ainvs), std::get<3>(*v2_restiny_t::As), std::get<3>(*v2_restiny_t::Ainvs)));
