...
/*
 * File:   ssbm-q33_early.cpp
 * Author: Till Kolditz <till.kolditz@gmail.com>
 *
 * Created on 20. June 2017, 13:14
 */
...
int main(
        int argc,
        char** argv) {
    ssb::init(argc, argv, "SSBM Query 3.3 Early Detection");
    ...
    for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
        ...
        // 0) Eager Check
        MEASURE_OP_TUPLE(tupleCK, checkAndDecodeAN(batCKenc));
        CLEAR_CHECKANDDECODE_AN(tupleCK);
        auto batCK = std::get<0>(tupleCK);
        ...
        MEASURE_OP(bat13, (select<std::greater_equal, std::less_equal, ahead::and_is>(batDY, 1992, 1997))); // OID date | d_year
