...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < CONFIG.NUM_RUNS; ++i) {
        ...
        MEASURE_OP(bat1, ahead::bat::ops::select<std::less>(batLQenc, 25 * batLQenc->tail.metaData.AN_A)); // lo_quantity < 25
        MEASURE_OP(bat2, ahead::bat::ops::select(batLDenc, 1 * batLDenc->tail.metaData.AN_A, 3 * batLDenc->tail.metaData.AN_A)); // lo_discount between 1 and 3
        ...
        MEASURE_OP(bat7, ahead::bat::ops::select<std::equal_to>(batDYenc, 1993 * batDYenc->tail.metaData.AN_A)); // d_year = 1993
        ...
        MEASURE_OP_TUPLE(tupleF, ahead::bat::ops::checkAndDecodeAN(batD));CLEAR_CHECKANDDECODE_AN(tupleF);
        delete batD;
        MEASURE_OP_TUPLE(tupleG, ahead::bat::ops::checkAndDecodeAN(batE));CLEAR_CHECKANDDECODE_AN(tupleG);
        delete batE;
        MEASURE_OP(batF, ahead::bat::ops::aggregate_mul_sum<v2_bigint_t>(std::get<0>(tupleF), std::get<0>(tupleG)));
