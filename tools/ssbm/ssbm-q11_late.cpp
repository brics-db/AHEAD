...
main (int argc, char** argv) {
    ...
    for (size_t i = 0; i < CONFIG.NUM_RUNS; ++i) {
        ...
        MEASURE_OP(bat1, v2::bat::ops::select<std::less>(batLQenc, 25 * batLQenc->tail.metaData.AN_A)); // lo_quantity < 25
        MEASURE_OP(bat2, v2::bat::ops::select(batLDenc, 1 * batLDenc->tail.metaData.AN_A, 3 * batLDenc->tail.metaData.AN_A)); // lo_discount between 1 and 3
        ...
        MEASURE_OP(bat7, v2::bat::ops::select<std::equal_to>(batDYenc, 1993 * batDYenc->tail.metaData.AN_A)); // d_year = 1993
        ...
        delete batE;
        MEASURE_OP(batF, v2::bat::ops::aggregate_mul_sum_SSE<v2_bigint_t>(std::get<0>(tupleF), std::get<0>(tupleG)));
