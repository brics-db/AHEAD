...
int main(
    ...
    return ssb::run(argc, argv, [] (int argc, char ** argv) -> int {
        ssb::init(argc, argv, "SSBM Query 1.1 Late Detection");
        ...
        for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
            ...
            MEASURE_OP(bat2, (select<std::greater_equal, std::less_equal, ahead::and_is>(batLDenc, 1 * batLDenc->tail.metaData.AN_A, 3 * batLDenc->tail.metaData.AN_A)));// lo_discount between 1 and 3
            ...
            MEASURE_OP(batE, matchjoin(bat5, batLDenc));
            delete bat5;
            ...
            MEASURE_OP_TUPLE(tupleG, checkAndDecodeAN(batD));
            CLEAR_CHECKANDDECODE_AN(tupleG);
            auto batG = std::get<0>(tupleG);
            delete batD;
            MEASURE_OP_TUPLE(tupleH, checkAndDecodeAN(batF));
            CLEAR_CHECKANDDECODE_AN(tupleH);
            auto batH = std::get<0>(tupleH);
            delete batF;

            // result
            MEASURE_OP(batI, aggregate_mul_sum<v2_bigint_t>(batG, batH));
            delete batG;
            delete batH;
