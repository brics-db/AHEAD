...
int main(
    ...
    ssb::loadTables( {"customerAN", "lineorderAN", "supplierAN", "dateAN"}, "SSBM Q3.2:\n"
    ...
    for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
        ...
        MEASURE_OP_TUPLE(tuple23,
                hashjoinAN(std::get<0>(tuple22), std::get<0>(tuple4), std::get<v2_resoid_t::As->size() - 1>(*v2_resoid_t::As), std::get<v2_resoid_t::Ainvs->size() - 1>(*v2_resoid_t::Ainvs))); // RESOID | RESOID supplier
        ...
        MEASURE_OP_TUPLE(tuple26,
                hashjoinAN(std::get<0>(tuple25), std::get<0>(tuple10), std::get<v2_resoid_t::As->size() - 1>(*v2_resoid_t::As), std::get<v2_resoid_t::Ainvs->size() - 1>(*v2_resoid_t::Ainvs))); // RESOID | RESOID customer
        ...
        MEASURE_OP_TUPLE(tuple29, hashjoinAN(std::get<0>(tuple28), bat16, std::get<v2_resoid_t::As->size() - 1>(*v2_resoid_t::As), std::get<v2_resoid_t::Ainvs->size() - 1>(*v2_resoid_t::Ainvs))); // RESOID | RESOID date
        delete bat16;
