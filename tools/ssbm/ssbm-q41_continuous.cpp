...
/* 
 * File:   ssbm-q41_continuous.cpp
 * Author: Till Kolditz <till.kolditz@gmail.com>
 *
 * Created on 23. June 2017, 11:06
 */
...
int main(
        int argc,
        char** argv) {
    ssb::init(argc, argv, "SSBM Query 4.1 Continuous Detection");
    ...
    for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
        ...
        MEASURE_OP_TUPLE(tuple5, hashjoinAN(batLP, std::get<0>(tuple4), std::get<v2_resoid_t::As->size() - 1>(*v2_resoid_t::As), std::get<v2_resoid_t::Ainvs->size() - 1>(*v2_resoid_t::Ainvs))); // OID lineorder | OID supplier
        ...
        MEASURE_OP_TUPLE(tuple25, fetchjoinAN(bat21, batLO)); // VOID | lo_orderdate
        delete bat21;
        ...
        MEASURE_OP_TUPLE(tupleAY, fetchjoinAN(bat31, batDY)); // VOID | d_year !!!
        delete bat31;
        ...
        MEASURE_OP_TUPLE(tupleRY, fetchjoinAN(std::get<1>(tupleGN), std::get<0>(tupleAY)));
