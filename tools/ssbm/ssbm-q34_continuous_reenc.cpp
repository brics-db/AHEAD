...
/*
 * File:   ssbm-q34_continuous_reenc.cpp
 * Author: Till Kolditz <till.kolditz@gmail.com>
 *
 * Created on 23. June 2017, 09:58
 */
...
int main(
        int argc,
        char** argv) {
    ssb::init(argc, argv, "SSBM Query 3.4 Continuous Detection With Reencoding");

    ssb::loadTables( {"customerAN", "lineorderAN", "supplierAN", "dateAN"}, "SSBM Q3.4:\n"
    ...
    for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
        ...
        decltype(matchjoinAN(bat3, bat2)) tupleX;
        ...
        // d_yearmonth = 'Dec1997'
        MEASURE_OP_TUPLE(tuple13, selectAN<std::equal_to>(batDM, const_cast<str_t>("Dec1997"))); // OID date | d_yearmonth
        CLEAR_SELECT_AN(tuple13);
        auto bat14 = std::get<0>(tuple13)->mirror_head(); // OID date | OID date
        delete std::get<0>(tuple13);
        MEASURE_OP_TUPLE(tuple15, matchjoinAN(bat14, batDD, std::get<7>(*v2_resoid_t::As), std::get<7>(*v2_resoid_t::Ainvs), std::get<11>(*v2_resint_t::As), std::get<11>(*v2_resint_t::Ainvs))); // OID date | d_datekey
        delete bat14;
        ...
        MEASURE_OP_TUPLE(tuple18, matchjoinAN(bat17, batLO, std::get<6>(*v2_resoid_t::As), std::get<6>(*v2_resoid_t::Ainvs), std::get<10>(*v2_resint_t::As), std::get<10>(*v2_resint_t::Ainvs))); // OID lineorder | lo_orderdate
        delete bat17;
        ...
        delete bat20;
        MEASURE_OP_TUPLE(tupleAR, fetchjoinAN(bat21, batLR, std::get<9>(*v2_resint_t::As), std::get<9>(*v2_resint_t::Ainvs))); // VOID | lo_revenue !!!
        CLEAR_FETCHJOIN_AN(tupleAR);
        MEASURE_OP_TUPLE(tuple22, fetchjoinAN(bat21, batLS, std::get<8>(*v2_resint_t::As), std::get<8>(*v2_resint_t::Ainvs))); // VOID | lo_suppkey
        ...
        MEASURE_OP_TUPLE(tuple25, fetchjoinAN(bat21, batLC, std::get<7>(*v2_resint_t::As), std::get<7>(*v2_resint_t::Ainvs))); // VOID | lo_custkey
        CLEAR_FETCHJOIN_AN(tuple25);
        MEASURE_OP_TUPLE(tuple26,
                hashjoinAN(std::get<0>(tuple25), std::get<0>(tuple10), std::get<1>(*v2_resoid_t::As), std::get<1>(*v2_resoid_t::Ainvs), std::get<14>(*v2_resoid_t::As),
        ...
        MEASURE_OP_TUPLE(tuple28, fetchjoinAN(bat21, batLO, std::get<6>(*v2_resint_t::As), std::get<6>(*v2_resint_t::Ainvs))); // VOID | lo_orderdate
        delete bat21;
        CLEAR_FETCHJOIN_AN(tuple28);
        MEASURE_OP_TUPLE(tuple29,
                hashjoinAN(std::get<0>(tuple28), bat16, std::get<13>(*v2_resoid_t::As), std::get<13>(*v2_resoid_t::Ainvs), std::get<12>(*v2_resoid_t::As), std::get<12>(*v2_resoid_t::Ainvs))); // OID | OID date
        delete bat16;
        ...
        MEASURE_OP_TUPLE(tupleAD, fetchjoinAN(bat30, batDY, std::get<14>(*v2_resshort_t::As), std::get<14>(*v2_resshort_t::Ainvs))); // VOID | d_year !!!
        delete bat30;
        CLEAR_FETCHJOIN_AN(tupleAD);

        // grouping
        MEASURE_OP_TUPLE(tupleGD, groupbyAN(std::get<0>(tupleAD), std::get<11>(*v2_resoid_t::As), std::get<11>(*v2_resoid_t::Ainvs)));
        CLEAR_GROUPBY_UNARY_AN(tupleGD);
        MEASURE_OP_TUPLE(tupleGS, groupbyAN(std::get<0>(tupleAS), std::get<0>(tupleGD), std::get<1>(tupleGD)->size(), std::get<10>(*v2_resoid_t::As), std::get<10>(*v2_resoid_t::Ainvs)));
        delete std::get<0>(tupleGD);
        delete std::get<1>(tupleGD);
        CLEAR_GROUPBY_BINARY_AN(tupleGS);
        MEASURE_OP_TUPLE(tupleGC, groupbyAN(std::get<0>(tupleAC), std::get<0>(tupleGS), std::get<1>(tupleGS)->size(), std::get<9>(*v2_resoid_t::As), std::get<9>(*v2_resoid_t::Ainvs)));
        ...
        MEASURE_OP_TUPLE(tupleRD, fetchjoinAN(std::get<1>(tupleGC), std::get<0>(tupleAD), std::get<13>(*v2_resshort_t::As), std::get<13>(*v2_resshort_t::Ainvs)));
