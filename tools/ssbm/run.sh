...
for NUM in "${IMPLEMENTED[@]}"; do
    ...
    for type in ${BASE2} ${BASE2}_eager ${BASE2}_lazy ${BASE2}_encoded; do
        if [[ -e ${type} ]]; then
            ...
            for sf in 1; do
                echo -n " sf${sf}"
                ./${type} ${DBLOC}/sf-${sf} 1>>${FILEOUT} 2>>${FILEERR}
            #./${type} ${DBLOC}/sf-${sf}
            done
            echo " done."
        fi
...
#echo "" >lineorder_size.out; for sf in `seq 1 10`; do date +%T | tr -d '\n'; echo -n ": Scale Factor ${sf}... "; echo "Scale Factor ${sf}" >>lineorder_size.out; ./lineorder_size ${DBLOC}/sf-${sf} >>lineorder_size.out; date +%T | tr -d '\n'; echo ": Done!"; done
