...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < CONFIG.NUM_RUNS; ++i) {
        ...
        MEASURE_OP(bat1, ahead::bat::ops::select<std::less>(std::get<0>(tupleLQ), 25)); // lo_quantity < 25
        delete std::get<0>(tupleLQ);
        MEASURE_OP(bat2, ahead::bat::ops::select(std::get<0>(tupleLD), 1, 3)); // lo_discount between 1 and 3
        ...
        MEASURE_OP(bat7, ahead::bat::ops::select<std::equal_to>(std::get<0>(tupleDY), 1993)); // d_year = 1993
