...
int main(
        int argc,
        char** argv) {
    ssb::init(argc, argv, "SSBM Query 1.1 DMR Parallel");

    ssb::loadTables( {"date", "lineorder"}, "SSBM Q1.1:\n"
...
#pragma omp parallel for
        for (size_t k = 0; k < DMR::modularity; ++k) {
            ...
            MEASURE_OP(bat2, (select<std::greater_equal, std::less_equal, ahead::and_is>(batLDs[k], 1, 3))); // lo_discount between 1 and 3
