...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < CONFIG.NUM_RUNS; ++i) {
        ...
        MEASURE_OP_PAIR(pair1,
                (v2::bat::ops::selectAN(batLQenc, static_cast<restiny_t>(26) * batLQenc->tail.metaData.AN_A, static_cast<restiny_t>(35) * batLQenc->tail.metaData.AN_A, std::get<6>(*v2_restiny_t::As),
                        std::get<6>(*v2_restiny_t::Ainvs)))); // lo_quantity between 26 and 35
        delete pair1.second;
        MEASURE_OP_PAIR(pair2,
                (v2::bat::ops::selectAN(batLDenc, static_cast<restiny_t>(4) * batLDenc->tail.metaData.AN_A, static_cast<restiny_t>(6) * batLDenc->tail.metaData.AN_A, std::get<5>(*v2_restiny_t::As),
        ...
        MEASURE_OP_PAIR(pair7,
                (v2::bat::ops::selectAN<std::equal_to>(batDYenc, static_cast<resint_t>(199401) * batDYenc->tail.metaData.AN_A, std::get<14>(*v2_resshort_t::As), std::get<14>(*v2_resshort_t::Ainvs)))); // d_yearmonthnum = 199401
        if (pair7.second)
            delete pair7.second;
        auto bat8 = pair7.first->mirror_head(); // prepare joined selection over d_year and d_datekey
        delete pair7.first;
        MEASURE_OP_TUPLE(tuple9,
                (v2::bat::ops::matchjoinAN(bat8, batDDenc, std::get<11>(*v2_resoid_t::As), std::get<11>(*v2_resoid_t::Ainvs), std::get<14>(*v2_resint_t::As), std::get<14>(*v2_resint_t::Ainvs)))); // only those d_datekey where d_year...
        delete bat8;
        ...
        MEASURE_OP_TUPLE(tupleB,
                (v2::bat::ops::hashjoinAN(std::get<0>(tuple6), batA, std::get<10>(*v2_resoid_t::As), std::get<10>(*v2_resoid_t::Ainvs), std::get<9>(*v2_resoid_t::As), std::get<9>(*v2_resoid_t::Ainvs)))); // only those lineorders where lo_quantity... and lo_discount... and d_year...
        ...
        MEASURE_OP_TUPLE(tupleD,
                (v2::bat::ops::matchjoinAN(batC, batLEenc, std::get<8>(*v2_resoid_t::As), std::get<8>(*v2_resoid_t::Ainvs), std::get<13>(*v2_resint_t::As), std::get<13>(*v2_resint_t::Ainvs))));CLEAR_HASHJOIN_AN(
                tupleD);
        MEASURE_OP_TUPLE(tupleE,
                (v2::bat::ops::matchjoinAN(batC, std::get<0>(tuple4), std::get<7>(*v2_resoid_t::As), std::get<7>(*v2_resoid_t::Ainvs), std::get<3>(*v2_restiny_t::As),
