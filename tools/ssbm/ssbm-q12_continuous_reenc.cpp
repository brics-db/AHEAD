...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
        ...
        MEASURE_OP_TUPLE(tupleD, (matchjoinAN(batC, batLEenc, std::get<8>(*v2_resoid_t::As), std::get<8>(*v2_resoid_t::Ainvs), std::get<13>(*v2_resint_t::As), std::get<13>(*v2_resint_t::Ainvs))));CLEAR_HASHJOIN_AN(
                tupleD);
