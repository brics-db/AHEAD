...
int main(
    ...
    return ssb::run(argc, argv, [] (int argc, char ** argv) -> int {
        ...
        ssb::loadTables( {"dateAN", "lineorderAN"}, "SSBM Q1.2:\n"
        ...
        for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
            ...
            MEASURE_OP_PAIR(pair2,
                    (selectAN<std::greater_equal, std::less_equal, ahead::and_is>(batLDenc, 4ull * batLDenc->tail.metaData.AN_A, 6ull * batLDenc->tail.metaData.AN_A, std::get<5>(*v2_restiny_t::As),
            ...
            MEASURE_OP_TUPLE(tupleE, (matchjoinAN(bat5, batLDenc, std::get<7>(*v2_resoid_t::As), std::get<7>(*v2_resoid_t::Ainvs), std::get<2>(*v2_restiny_t::As), std::get<2>(*v2_restiny_t::Ainvs))));
            CLEAR_JOIN_AN(tupleE);
            delete bat5;
            MEASURE_OP_TUPLE(tupleF,
                    (matchjoinAN(batC, std::get<0>(tupleE), std::get<6>(*v2_resoid_t::As), std::get<6>(*v2_resoid_t::Ainvs), std::get<1>(*v2_restiny_t::As), std::get<1>(*v2_restiny_t::Ainvs))));
            delete batC;
