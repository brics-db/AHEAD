...
main (int argc, char** argv) {
    ...
    for (size_t i = 0; i < CONFIG.NUM_RUNS; ++i) {
        ...
        MEASURE_OP(bat5, v2::bat::ops::hashjoin(std::get<0>(tupleLS), bat4)); // OID lineorder | OID supplier
        ...
        delete bat5;
        MEASURE_OP(bat7, v2::bat::ops::matchjoin(bat6, std::get<0>(tupleLP))); // OID lineorder | lo_partkey (where s_region = 'AMERICA')
        delete bat6;

        // p_category = 'MFGR#12'
        MEASURE_OP(bat8, v2::bat::ops::select<std::equal_to>(batPC, const_cast<str_t>("MFGR#12"))); // OID part | p_category
        // p_brand = 'MFGR#121'
        ...
        delete batC;
        MEASURE_OP(batF, v2::bat::ops::matchjoin(batE, std::get<0>(tupleLO))); // OID lineorder | lo_orderdate (where ...)
        delete batE;
        ...
        MEASURE_OP(batX, v2::bat::ops::matchjoin(batW, std::get<0>(tupleLP))); // OID lineorder | lo_partkey
        ...
        delete batX;
        delete batY;
        MEASURE_OP(batA1, v2::bat::ops::hashjoin(batZ, batPB)); // OID lineorder | p_brand
        delete batZ;

        MEASURE_OP(batA2, v2::bat::ops::hashjoin(batI, std::get<0>(tupleDY))); // OID lineorder | d_year
        delete batI;
        delete std::get<0>(tupleDY);

        MEASURE_OP(batA3, v2::bat::ops::matchjoin(batW, std::get<0>(tupleLR))); // OID lineorder | lo_revenue (where ...)
        delete batW;
