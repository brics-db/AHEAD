...
/*
 * File:   ssbm-q43_continuous_reenc.cpp
 * Author: Till Kolditz <till.kolditz@gmail.com>
 *
 * Created on 28. June 2017, 16:18
 */
...
int main(
        int argc,
        char** argv) {
    ssb::init(argc, argv, "SSBM Query 4.3 Continuous Detection With Reencoding");
    ...
    for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
        ...
        MEASURE_OP_TUPLE(tuple5,
                hashjoinAN(batLP, std::get<0>(tuple4), std::get<13>(*v2_resoid_t::As), std::get<13>(*v2_resoid_t::Ainvs), std::get<12>(*v2_resoid_t::As), std::get<12>(*v2_resoid_t::Ainvs))); // OID lineorder | OID supplier
        ...
        MEASURE_OP_TUPLE(tuple21,
                (selectAN<std::equal_to, std::equal_to, OR>(batDY, 1997ull * batDY->tail.metaData.AN_A, 1998ull * batDY->tail.metaData.AN_A, std::get<14>(*v2_resshort_t::As),
        ...
        MEASURE_OP_TUPLE(tuple24, matchjoinAN(bat23, bat22, std::get<11>(*v2_resint_t::As), std::get<11>(*v2_resint_t::Ainvs), std::get<8>(*v2_resoid_t::As), std::get<8>(*v2_resoid_t::Ainvs))); // d_datekey | OID date
        delete bat22;
        delete bat23;
        CLEAR_JOIN_AN(tuple24);
        // reduce number of lo_datekey joinpartners
        MEASURE_OP_TUPLE(tuple25, matchjoinAN(bat13, batLO, std::get<7>(*v2_resoid_t::As), std::get<7>(*v2_resoid_t::Ainvs), std::get<10>(*v2_resint_t::As), std::get<10>(*v2_resint_t::Ainvs))); // OID lineorder | lo_datekey
        delete bat13;
        CLEAR_JOIN_AN(tuple25);
        // lo_orderdate = d_datekey
        MEASURE_OP_TUPLE(tuple26,
                hashjoinAN(std::get<0>(tuple25), std::get<0>(tuple24), std::get<6>(*v2_resoid_t::As), std::get<6>(*v2_resoid_t::Ainvs), std::get<5>(*v2_resoid_t::As),
                        std::get<5>(*v2_resoid_t::Ainvs))); // OID lineorder | OID date
        ...
        delete bat27;
        // profit
        MEASURE_OP_TUPLE(tupleAR, fetchjoinAN(bat28, batLR, std::get<10>(*v2_resint_t::As), std::get<10>(*v2_resint_t::Ainvs))); // VOID | lo_revenue
        CLEAR_FETCHJOIN_AN(tupleAR);
        MEASURE_OP_TUPLE(tupleAS, fetchjoinAN(bat28, batLSC, std::get<9>(*v2_resint_t::As), std::get<9>(*v2_resint_t::Ainvs))); // VOID | lo_supplycost
        CLEAR_FETCHJOIN_AN(tupleAS);
        MEASURE_OP_TUPLE(tupleAP, (arithmeticAN<SUB, v2_resint_t>(std::get<0>(tupleAR), std::get<0>(tupleAS), std::get<8>(*v2_resint_t::As), std::get<8>(*v2_resint_t::Ainvs)))); // VOID | lo_revenue - lo_supplycost (profit !!!)
        ...
        MEASURE_OP_TUPLE(tuple30,
                hashjoinAN(std::get<0>(tuple29), std::get<0>(tuple10), std::get<4>(*v2_resoid_t::As), std::get<4>(*v2_resoid_t::Ainvs), std::get<3>(*v2_resoid_t::As),
                        std::get<3>(*v2_resoid_t::Ainvs))); // OID | OID customer
        ...
        MEASURE_OP_TUPLE(tuple34,
                hashjoinAN(std::get<0>(tuple32), std::get<0>(tuple24), std::get<2>(*v2_resoid_t::As), std::get<2>(*v2_resoid_t::Ainvs), std::get<1>(*v2_resoid_t::As),
                        std::get<1>(*v2_resoid_t::Ainvs))); // OID | OID date
        ...
        MEASURE_OP_TUPLE(tuple38,
                hashjoinAN(std::get<0>(tuple36), bat37, std::get<15>(*v2_resoid_t::As), std::get<15>(*v2_resoid_t::Ainvs), std::get<14>(*v2_resoid_t::As), std::get<14>(*v2_resoid_t::Ainvs))); // OID | OID partkey
        ...
        MEASURE_OP_TUPLE(tupleGC, groupbyAN(std::get<0>(tupleAC), std::get<0>(tupleGY), std::get<1>(tupleGY)->size(), std::get<12>(*v2_resoid_t::As), std::get<12>(*v2_resoid_t::Ainvs)));
        delete std::get<0>(tupleGY);
        delete std::get<1>(tupleGY);
        CLEAR_GROUPBY_BINARY_AN(tupleGC);
        MEASURE_OP_TUPLE(tupleGB, groupbyAN(std::get<0>(tupleAB), std::get<0>(tupleGC), std::get<1>(tupleGC)->size(), std::get<11>(*v2_resoid_t::As), std::get<11>(*v2_resoid_t::Ainvs)));
        ...
        MEASURE_OP_TUPLE(tupleRY, fetchjoinAN(std::get<1>(tupleGB), std::get<0>(tupleAY), std::get<12>(*v2_resshort_t::As), std::get<12>(*v2_resshort_t::Ainvs)));
