...
/*
 * File:   ssbm-q42_continuous.cpp
 * Author: Till Kolditz <till.kolditz@gmail.com>
 *
 * Created on 27. June 2017, 10:48
 */
...
int main(
        int argc,
        char** argv) {
    ssb::init(argc, argv, "SSBM Query 4.2 Continuous Detection");
    ...
    for (size_t i = 0; i < ssb::ssb_config.NUM_RUNS; ++i) {
        ...
        MEASURE_OP_TUPLE(tuple5,
                hashjoinAN(batLP, std::get<0>(tuple4), std::get<15>(*v2_resoid_t::As), std::get<15>(*v2_resoid_t::Ainvs), std::get<0>(tuple4)->tail.metaData.AN_A,
                        std::get<0>(tuple4)->tail.metaData.AN_Ainv)); // OID lineorder | OID part
        ...
        MEASURE_OP_TUPLE(tuple17, matchjoinAN(bat16, bat15)); // c_custkey | OID customer
        delete bat15;
        delete bat16;
        CLEAR_JOIN_AN(tuple17);
        // reduce number of l_custkey joinpartners
        MEASURE_OP_TUPLE(tuple18, matchjoinAN(bat13, batLC)); // OID lineorder | lo_custkey
        delete bat13;
        CLEAR_JOIN_AN(tuple18);
        // lo_custkey = c_custkey
        MEASURE_OP_TUPLE(tuple19, hashjoinAN(std::get<0>(tuple18), std::get<0>(tuple17))); // OID lineorder | OID customer
        ...
        MEASURE_OP_TUPLE(tuple25, matchjoinAN(bat20, batLO)); // OID lineorder | lo_datekey
        delete bat20;
        ...
        MEASURE_OP_TUPLE(tuple30,
                hashjoinAN(std::get<0>(tuple29), std::get<0>(tuple10), std::get<v2_resoid_t::As->size() - 1>(*v2_resoid_t::As), std::get<v2_resoid_t::Ainvs->size() - 1>(*v2_resoid_t::Ainvs))); // OID | OID customer
        ...
        MEASURE_OP_TUPLE(tuple34,
                hashjoinAN(std::get<0>(tuple32), std::get<0>(tuple24), std::get<v2_resoid_t::As->size() - 1>(*v2_resoid_t::As), std::get<v2_resoid_t::Ainvs->size() - 1>(*v2_resoid_t::Ainvs))); // OID | OID date
        ...
        MEASURE_OP_TUPLE(tupleGN, groupbyAN(std::get<0>(tupleAN), std::get<0>(tupleGY), std::get<1>(tupleGY)->size()));
        delete std::get<0>(tupleGY);
        delete std::get<1>(tupleGY);
        CLEAR_GROUPBY_BINARY_AN(tupleGN);
        MEASURE_OP_TUPLE(tupleGC, groupbyAN(std::get<0>(tupleAC), std::get<0>(tupleGN), std::get<1>(tupleGN)->size()));
        ...
        MEASURE_OP_TUPLE(tupleRY, fetchjoinAN(std::get<1>(tupleGC), std::get<0>(tupleAY)));
